...
(defvar mac-key-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "s-f") (key-binding (kbd "C-s")))
    ...
    (define-key map (kbd "s-l") 'select-current-line)
    ;; (define-key map (kbd "s-t") 'mac-font-panel-mode)
    ...
    (define-key map (kbd "s-\?") 'info)
    (define-key map (kbd "s-/") 'comment-toggle)
    ...
    (define-key map (kbd "<C-down-mouse-1>") 'mac-key-context-menu)
    (define-key map (kbd "<mouse-3>") 'mac-key-context-menu)
    (define-key map (kbd "C-<mouse-1>") 'mac-key-context-menu)
    (define-key map (kbd "A-S-<mouse-1>") 'mouse-buffer-menu)
    (define-key map (kbd "S-<down-mouse-1>") 'mac-key-shift-mouse-select)
    (define-key map (kbd "s-h") 'ns-do-hide-emacs)
    (define-key map (kbd "s-H") 'ns-do-hide-others)
