...
** DONE Make yanking indentation work on a newline inside a form in parinfer (mac-key-mode's =clipboard-yank-and-indent= needs to work with parinfer's =parinfer-smart-yank:yank=. Maybe advise the latter? Could turn on parinfer-paren-mode while yanking) [0/2]
** TODO Update readme with new non-bare repo setup
Mostly done. Need to finish backup section
